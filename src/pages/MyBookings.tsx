...
export const MyBookings = ({ navigateTo }: MyBookingsProps) => {
  ...
  if (loading) {
    return (
      <div className="p-20 text-center font-bold text-text-muted">
        Loading bookings...
      </div>
    );
  }
        ...
        <div className="flex bg-white p-2 rounded-2xl border border-gray-200 shadow-sm shrink-0">
          {[
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab.id
            ...
            >
              ...
              <div className="flex-1 text-center xl:text-left space-y-2">
                ...
                <div className="flex items-center justify-center xl:justify-start gap-3 pt-2">
                  ...
                  <p className="text-sm font-bold">
                    Tutor ID:
                    <span className="text-primary underline underline-offset-4 decoration-2 decoration-primary/20 ml-2">
                      {booking.tutorId}
                    </span>
                  </p>
                </div>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 shrink-0 w-full xl:w-auto">
                {booking.status === "confirmed" && (
                  <button className="w-full sm:w-auto px-10 py-4 bg-primary text-white rounded-2xl text-xs font-black uppercase tracking-[0.2em] hover:bg-primary/90 transition-all shadow-xl shadow-primary/20">
                    Join Call
                  </button>
                )}
                ...
                {booking.status === "pending" ||
                  booking.status === "confirmed" ? (
                  <>
                    <button className="w-full sm:w-auto px-10 py-4 border-2 border-gray-100 rounded-2xl text-xs font-black text-text-muted hover:border-primary hover:text-primary uppercase tracking-[0.2em] bg-white transition-all">
                      Reschedule
                    </button>

                    <button
                      onClick={() => handleCancel(booking.bookingId)}
                      className="w-full sm:w-auto px-10 py-4 border-2 border-transparent hover:border-error hover:text-error rounded-2xl text-xs font-black text-text-muted uppercase tracking-[0.2em] transition-all"
                    >
                      Cancel
                    </button>
                  </>
